codeunit 50119 prPayrollProcessing
{
    ...
    var
        ...
        curhslReliefAmount: Decimal;
        CurHouseLyRelief: Decimal;
        HousingLevyPercentage: decimal;
        HslReliefAmount: decimal;
    ...
    begin
        ...
        "HousingLevyRelief%" := VitalSetup."Housing Levy Relief %";
        HousingLevyPercentage := VitalSetup."Housing Levy%";
    ...
    begin
        ...
        JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::NSSF);
            ...
            if PaysHsl then begin
                CurHsLy := ((HousingLevyPercentage / 100) * curGrossTaxable);
                curTransAmount := CurHsLy;

                strTransDescription := 'Housing Levy';
                TGroup := 'STATUTORIES';
                TGroupOrder := 7;
                TSubGroupOrder := 2;
                fnUpdatePeriodTrans(strEmpCode, 'HSLY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                 curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept,
                 NHIFEMPyee, JournalPostAs::Credit, JournalPostingType::
                 "G/L Account", '', CoopParameters::none);

                IF blnPaysPaye THEN BEGIN
                    //Get Housing Levy  Relief

                    IF ("HousingLevyRelief%" > 0) THEN BEGIN


                        curhslReliefAmount := (("HousingLevyRelief%" / 100) * CurHsLy);



                        IF curhslReliefAmount > 0 THEN BEGIN
                            curTransAmount := curhslReliefAmount;
                            HslReliefAmount := curTransAmount;
            ...
            IF VitalSetup."Use NHIF" = true then begin
                IF blnPaysNhif THEN BEGIN
                    ...
                    END;
                END
            end else if VitalSetup."Use SHIF" = True then begin
                if blnPaysNhif then begin
                    curNHIF := Round(((VitalSetup."SHIF %" / 100) * curGrossPay), 0.01, '=');
                    IF curNHIF < vitalsetup."SHIF Min Amount" then
                        curTransAmount := vitalsetup."SHIF Min Amount" else
                        curTransAmount := curNHIF;
                    NHIFEMPyee := PostingGroup."NHIF Employee Account";
                ...
                IF curPensionCompany > 0 THEN BEGIN
                    ...
                    IF curExcessPension > 0 THEN BEGIN
                        ...
                        //     TGroup := 'DEDUCTIONS';
                        //     TGroupOrder := 6;
                        //     TSubGroupOrder := 5; //CHANGE STATUTORY DEDUCTIONS TO STATUTORIES-HOSEA //STATUTORIES TO DEDUCTIONS// change TGroupOrder :=7 to 6
                        ...
                        TGroup := 'DEDUCTIONS';
                        TGroupOrder := 6;
                        TSubGroupOrder := 6; //CHANGE STATUTORY DEDUCTIONS TO STATUTORIES-HOSEA //STATUTORIES TO DEDUCTIONS // change TGroupOrder :=7 to 6
     ...
     */
                ...
                if HREmp2.Get(strEmpCode) then begin
                    if HREmp2."Physical Disability" = true then begin
                        if curTaxablePay >= curDisabledLimit then begin
                            //if curGrossPay >= curDisabledLimit then begin
                ...
                if (curInsuranceReliefAmount + fNHIFReliefAmount) > curMaximumRelief then begin
                    ...
                    curPAYE := curTaxCharged - (curReliefPersonal + curInsuranceReliefAmount + fNHIFReliefAmount + curReliefMorgage + HslReliefAmount);
                    //  end else begin
                ...
                curPAYE := curPAYE + curTaxOnExcessPension;
               ...
               curPAYE := curTaxCharged - (curReliefPersonal + curInsuranceReliefAmount);
                ...
                curNetPay := curGrossPay - (curDefinedContrib + curNHIF + curPAYE + curPayeArrears + curTotalDeductions + IsCashBenefit + CurHsLy);
