report 50013 "Payroll Variance Reports"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("PRL-Period Transactions"; "PRL-Period Transactions")
        {
            ...
            begin
                if StartPeriods = 0D then Error('Specify the end period!');
                if EndPeriods = 0D then Error('Specify the start period!');
    ...
    begin
        ...
        if startPeriods = 0D then Error('Specify the end period!');
        objPeriod.Reset;
        objPeriod.SetRange("Date Opened", StartPeriods);
        if objPeriod.find('-') then
            PeriodName1 := objPeriod."Period Name";

        objPeriod.Reset;
        objPeriod.SetRange("Date Opened", EndPeriods);
        if objPeriod.find('-') then
            PeriodName2 := objPeriod."Period Name";
    end;
