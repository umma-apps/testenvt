report 50010 "Housing Levy Report"
{
    Caption = 'Housing Levy Report';
    DefaultLayout = RDLC;
    RDLCLayout = './payroll/Report/SSR/HousingLevyReport.rdl';
    ...
    dataset
    {
        dataitem(prlTransactions; "PRL-Period Transactions")
        {
            ...
            // DataItemTableView = SORTING("Group Order") ORDER(Ascending) WHERE("Transaction Code" = FILTER('TR-066'));
            DataItemTableView = SORTING("Group Order") ORDER(Ascending) WHERE("Transaction Code" = FILTER('HSLY'), "Period Filter" = FILTER(''));
            ...
            begin
                ...
                if prlEmpTrans.Find('-') then begin
                    ...
                    Clear(empContr);
                    Clear(totalContr);
                    basicsal := (100 / 0.015) * prlEmpTrans.Amount;
                    empContr := (0.015 / 100) * basicsal;
                ...
                Gtoto := totalContr;//+PRLEmployerDeductions.Amount;
