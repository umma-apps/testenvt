report 52178892 "Payment Voucher2"
{
    ...
    dataset
    {
        dataitem(PaymentHeader; "FIN-Payments Header")
        {
            ...
            dataitem("<Payment Line>"; "FIN-Payment Line")
            {
                ...
                column(Account_Name; "Account Name" + '-' + DimValName + '-' + DivisonName + '-' + SubDivisionName + '-' + DepartmentName)
                {
                ...
                column(PAYE_Amount; "PAYE Amount")
                {
                ...
                column(DivisonName; DivisonName) { }
                column(SubDivisionName; SubDivisionName) { }
                ...
                begin
                    DimVal.RESET;
                    DimVal.SETRANGE(DimVal."Dimension Code", 'CAMPUS');
                    DimVal.SETRANGE(DimVal.Code, "Global Dimension 1 Code");
                    DimValName := '';
                    IF DimVal.FINDFIRST THEN BEGIN
                        DimValName := DimVal.Name;
                        DimVal.RESET;
                        DimVal.SETRANGE(DimVal."Dimension Code", 'DIVISION');
                        DimVal.SETRANGE(DimVal.Code, "Shortcut Dimension 2 Code");
                        DivisonName := '';
                        IF DimVal.FINDFIRST THEN BEGIN
                            DivisonName := DimVal.Name;
                        end;
                        DimVal.RESET;
                        DimVal.SETRANGE(DimVal."Dimension Code", 'SCHOOL-SUB DIVISION');
                        DimVal.SETRANGE(DimVal.Code, "Shortcut Dimension 3 Code");
                        SubDivisionName := '';
                        IF DimVal.FINDFIRST THEN BEGIN
                            SubDivisionName := DimVal.Name;
                            DimVal.RESET;
                            DimVal.SETRANGE(DimVal."Dimension Code", 'PROGRAM-DEPARTMENT');
                            DimVal.SETRANGE(DimVal.Code, "Shortcut Dimension 4 Code");
                            DepartmentName := '';
                            IF DimVal.FINDFIRST THEN BEGIN
                                DepartmentName := DimVal.Name;
                            end;
                        end;
                    END;
                    ...
                    if "<Payment Line>"."PAYE Amount" <> 0 then begin
                        payeAcc := '21112006';
                        payeNm := 'PAYE Deductions Payable';
                    end;
                end;
            }
            ...
            begin
                ...
                if (PaymentHeader.Status <> Status::Approved) then
                    Error('PV must be fully Approved First');
    ...
    var
        ...
        DimValName: Text[100];
        DivisonName: Text[100];
        SubDivisionName: Text[100];
